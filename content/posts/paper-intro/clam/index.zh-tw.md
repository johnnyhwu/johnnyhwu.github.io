...
這篇論文提出了一個名為 **CLAM (CLarify-if-AMbiguous)** 的框架，旨在解決大型語言模型 (LLM) 在面對定義不清 (Ambiguous) 的問題時，傾向於過度自信地「盲目猜測」或產生幻覺的痛點。如果這個問題聽起來很熟悉，我們的 [INTENT-SIM](../intent-sim/) 文章從另一個角度處理了完全相同的「LLM 什麼時候該提出釐清問題」議題 — 透過模擬使用者可能的意圖，而不是三階段漏斗。
