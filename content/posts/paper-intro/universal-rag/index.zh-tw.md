...
我們過去習慣於追求一個萬能的「對齊向量空間」，希望文字、圖片、影片能在同一個維度下共存。但 UniversalRAG 告訴我們：**強行對齊會帶來偏見**。透過在語意層級進行路由，讓每種數據保留其最原始、最專精的表示方式，反而能獲得更公平、更準確的檢索結果。這種「解耦 (Decoupling) 」的思想，是解決複雜多模態問題的關鍵。有趣的是，同樣的哲學也出現在 [MRAG](../mrag/) 將時間推理與語意檢索解耦的做法中，只是套用在不同的異質性維度上。
