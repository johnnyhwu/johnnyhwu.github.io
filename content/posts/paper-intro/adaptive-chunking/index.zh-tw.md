...
<!-- UNRESOLVED IMAGE: img-012 -- 這個 manifest 項目（對應 Table 7，後處理前後的尺寸合規性對照）擷取出來的 PNG 只有附錄的跑頁標題文字（"Appendix I. Post-Processing Effect Details"），並沒有實際的表格內容，parser 似乎擷取錯了區域。需要人工從原始 PDF 重新截圖。 -->

具體的提升幅度：後處理前，LLM Regex 與語意切分器原本的合規率只有約 58% 與 48%，經過「過大重切」加「極小合併」雙管齊下後，兩者都飆升到 99% 以上。（來源：原始論文 Table 7 相關數據）
