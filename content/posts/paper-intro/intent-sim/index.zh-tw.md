...
今天要聊的這篇 Paper **"Clarify When Necessary" (NAACL 2025)**，就提出了一個非常優雅的解法。作者不只定義了什麼時候該問，更提出了一套名為 **INTENT-SIM** 的演算法，讓模型在「腦海中」模擬使用者的多種意圖，藉此精準判斷是否需要發起對話。如果想看看同一個問題的另一種解法，[CLAM](../clam/) 是用明確的 Detection-Questioning-Answering 漏斗來處理，而不是模擬意圖。
