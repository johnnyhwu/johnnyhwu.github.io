...
The per-iteration call-count formulas for the four methods are: WikiSkill is \( (1 + T_{\text{ReAct}}) \times (N_{\text{train}} / B) \), EvoSkill is \( 2 \times N_{\text{train}} / B \), SkillOpt is \( K_{\text{opt}} \times N_{\text{train}} / B \), and Trace2Skill is roughly \( N_{\text{train}} + (1 + 1/(c-1)) \times (N_{\text{train}} / B) + 1 \). In the paper's experiments, WikiSkill sets batch size equal to the training set size across every dataset, so \( N_{\text{train}} / B \) is always exactly 1, and the formula simplifies to \( 1 + T_{\text{ReAct}} \) — depending only on the number of ReAct rounds, completely independent of training set size, with \( T_{\text{ReAct}} \) falling roughly between 10 and 20 in the paper's experiments.
