...
lastmod: 2026-08-08
draft: false
description: "SketchVLM 論文導讀：不用重新訓練模型，只靠座標網格、XML 指令與貝茲曲線平滑化，就能讓 VLM 在原圖上疊加向量標註，把推理過程畫給你看，也讓答案變得可以驗證。"
...
- **特定任務微調**（如 ViLaSR、ThinkMorph）：在訓練過的任務上表現不錯，但換一個沒看過的任務類型，例如新的迷宮佈局，準確度就會崩掉，泛化能力差。（多模態模型的微調策略本身也是一個獨立的研究方向，可參考 [Tuning LayerNorm in Attention](../layernorm-tuning-multi-modal/)。）
